...
from statshed_cli.config import (
    Config,
    ConfigError,
    SubmitConfig,
    _find_config_file,
...
class TestFindConfigFile:
    ...
    def test_env_var_path(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """Test STATSHED_CONFIG environment variable."""
        config_file = tmp_path / "config.yaml"
        config_file.write_text("url: http://example.com")
    ...
    def test_env_var_not_found(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Test STATSHED_CONFIG with nonexistent file."""
        monkeypatch.setenv("STATSHED_CONFIG", "/nonexistent/config.yaml")
        with pytest.raises(ConfigError, match="Configuration file not found"):
            _find_config_file()

    def test_no_config_found(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Test when no config file is found."""
        monkeypatch.delenv("STATSHED_CONFIG", raising=False)
...
class TestConfigFromSources:
    ...
    def test_env_url_overrides_file(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """Test that STATSHED_URL env var overrides config file."""
        config_file = tmp_path / "config.yaml"
        config_file.write_text("url: http://file.example.com")
        monkeypatch.setenv("STATSHED_CONFIG", str(config_file))
        monkeypatch.setenv("STATSHED_URL", "http://env.example.com")
